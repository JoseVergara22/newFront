  ...
  getEstimateParts(ind: number) {
   ...
   upload() { 
    ...
    this.uploadService.uploadFileForkliftUpdate3(this.blobGlobal).then(res=>{
      ...
      this.estimateService.createEstimateFile(this.estimateCurrent.id,'','',1,  this.estimateCurrent.consecutive).then(data => {
        const resp: any = data;

        // PONER ENVIO EN ESTE PUNTO
      ...
      this.estimateService.sendEstimateEmailAmazon(//sendEstimateEmailAmazon
        ...
        this.emailsSend,this.comment,subjectTemp).then(data => {
        const resp: any = data;
        console.log('envio');
        console.log(resp);
        swal({
    ...
    sendEmailEstimate(){
      ...
      if(this.emailsSend.length>0){
        this.downloadPdf(this.estimateCurrent, 1);
      }else{
            swal({
...
if(this.filesImage[3]){
   ...
   console.log('extension');
   console.log(this.filesImage[3].ext);
...
img.onload = function() {
      ...
      height=height+10;
      doc.addImage(this.filesImage[0].content, this.filesImage[0].ext,  15,  height, 150,150);
      //doc.save('FirstPdf.pdf');
      //doc.addPage();
      height=height+150;
...
//----------------
  ...
  */
  ...
  var height=doc.autoTable.previous.finalY+10;
 /* if(doc.autoTable.previous.finalY+150>631){
    doc.addPage();
    height=100;
  }*/
  ...
  /*for (let i = 0; i < this.filesImage.length; i++) { 
    ...
    height=height+120;
    doc.addImage(this.filesImage[i].content, this.filesImage[i].ext, 15, height, 150, 150); 
    height=height+150;
  }*/
...
if(this.filesImage[1]){
  if(height+150>631){
   ...
   src2 = this.filesImage[1].url; 
   this.extImageTwo = this.filesImage[1].ext;
   doc.addImage( this.filesImage[1].content, this.filesImage[1].ext,  15,  height, 150,150);
}
...
if(this.filesImage[2]){
  if(height+150>631){
   ...
   src3 = this.filesImage[2].url; 
   this.extImageThree = this.filesImage[2].ext;
   doc.addImage( this.filesImage[2].content, this.filesImage[2].ext,  15,  height, 150,150);
}
if(this.filesImage[3]){
  if(height+150>631){
    doc.addPage();
   ...
   src4 = this.filesImage[3].url; 
   this.extImageFour = this.filesImage[3].ext;
   doc.addImage( this.filesImage[3].content, this.filesImage[3].ext,  15,  height, 150,150);
}
...
img.onload = function() {
      ...
      if(2<=imagesLong){
      img2.onload = function() {
        doc.addImage(img2, this.extImageTwo,  15, 300, 150,150);
...
this.getImgFromUrl(logo_url, function (img) {
  ...
  updateEstimate(row:any){
    console.log(row);
    this.router.navigateByUrl('master/estimateCustomerUpdate/'+row.id);
  }
...
updateForklift(forklift:any) {
  ...
  createEstimate(){
    this.router.navigateByUrl('master/estimateCustomer');
  }
  ...
  generalAlert(title:string,message:string,type:any){
    ...
    this.estimateService.getEstimateDetailFilesImages(Number(this.estimateId)).then(data => {
      ...
      for (let i = 0; i < this.filesEstimateImage.length; i++){
        ...
        console.log(this.filesEstimateImage[i].url);
   
        this.getBase64ImageFromUrl(this.filesEstimateImage[i].url)
        .then(result =>{
        //  console.log(result);
        //this.download(254);
        console.log(result);
        
        var ext=(this.filesEstimateImage[i].name).toString().split('.').pop();
        var url =this.filesEstimateImage[i].url;
          ...
          if(i== this.filesEstimateImage.length-1){
           ...
           console.log(ind);
            if(ind==0){
              ...
              this.download(ind);  // es la misma, si algo quitar la condición
            }
