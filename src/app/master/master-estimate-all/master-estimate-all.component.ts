...
export class MasterEstimateAllComponent extends NgbDatepickerI18n {
  ...
  checkHideCode = true;// false por defecto
  ...
  let ind=0; // 0 Solo para descargar y 1 para enviar;
   ...
   getEstimateWorkforce(ind: number) {
    if(this.estimateId){
      this.estimateService.getEstimateDetailsWorkforce(this.estimateId).then(data => {
        ...
        this.getFilesImage(this.estimateId);
   ...
   upload() { 
    ...
    sendEmailEstimate(){
      ...
      this.estimateService.sendEstimateEmail(
        this.estimateCurrent.id, this.emails, 107, this.message, this.subject).then(data => {
        const resp: any = data;
        console.log('envio');
        console.log(resp);
        swal({
