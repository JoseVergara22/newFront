  ...
  checkUncheckAllStatus(event:any){
    ...
    if(cont >=1){      
      ...
      this.reportService.showFilterDuration(params).then(data => {
        ...
        this.exportAsExcelFile(this.dataExcels,'Informe de Duración de Mantenimientos '+ fromD + ' - ' + untilD);
        swal.close();
    ...
    public exportAsExcelFile(rows: any[], excelFileName: string): void {
      if (rows.length > 0) {
        ...
        const workbook: XLSX.WorkBook = {Sheets: {'Info-Dura-Mantenimientos': worksheet}, SheetNames: ['Info-Dura-Mantenimientos']};
