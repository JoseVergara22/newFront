...
@Component({
  ...
  styleUrls: ['./master-time-out-forklift.component.scss',
  '../../../assets/icon/icofont/css/icofont.scss'],
  providers: [I18n, {provide: NgbDatepickerI18n, useClass: MasterTimeOutForkliftComponent}]
})
export class MasterTimeOutForkliftComponent extends NgbDatepickerI18n {
  ...
  getFilters() {
    ...
    console.log('.---------->'+params);
    this.reportService.showTimeOutForklift(params).then(data => {
      const resp: any = data;
      console.log(resp);
      ...
      this.exportAsExcelFile(this.dataExcels,'Informe de Tiempo Muerto de Equipos '+ fromD + ' - ' + untilD);
      swal.close();
  ...
  public exportAsExcelFile(rows: any[], excelFileName: string): void {
    if (rows.length > 0) {
      ...
      const workbook: XLSX.WorkBook = {Sheets: {'Info-TiempoMuerto': worksheet}, SheetNames: ['Info-TiempoMuerto']};
