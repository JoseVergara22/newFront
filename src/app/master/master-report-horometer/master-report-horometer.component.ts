...
@Component({
  ...
  styleUrls: ['./master-report-horometer.component.scss',
  '../../../assets/icon/icofont/css/icofont.scss'],
  providers: [I18n, {provide: NgbDatepickerI18n, useClass: MasterReportHorometerComponent}]
})
export class MasterReportHorometerComponent extends NgbDatepickerI18n {
  ...
  getFilters() {
    ...
    console.log('.---------->'+params);
    this.reportService.showHorometerForklift(params).then(data => {
      const resp: any = data;
      console.log(resp);
      ...
      this.exportAsExcelFile(this.dataExcels,'Informe de Horometro de Equipos '+ fromD + ' - ' + untilD);
      swal.close();
  ...
  public exportAsExcelFile(rows: any[], excelFileName: string): void {
    if (rows.length > 0) {
      ...
      const workbook: XLSX.WorkBook = {Sheets: {'Info-Horometro': worksheet}, SheetNames: ['Info-Horometro']};
